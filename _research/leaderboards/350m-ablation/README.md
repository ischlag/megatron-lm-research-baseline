...
| 2 | aurora-qkn-xsa-vresid | aurora-qkn-xsa | add `--value-residual` (ResFormer / value-residual learning) on top of XSA | Aurora + QK norm + XSA + V-residual | 1e-2 | **2.149** | **1.993** | [`20-aurora-qkn-xsa-vresid.sbatch`](runs/20-aurora-qkn-xsa-vresid.sbatch) | [69xzjzim](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/69xzjzim) | [`4d072497f`](https://github.com/ischlag/megatron-lm-research-baseline/tree/4d072497f) |
| 3 | aurora-qkn-dwa | aurora-qkn | add `--dwa` (DenseFormer / Depth-Weighted-Average across past block outputs); MBS 16 -> 8 to fit past-block activation cache | Aurora + QK norm + DWA | 1e-2 | 2.154 | 1.995 | [`19-aurora-qkn-dwa.sbatch`](runs/19-aurora-qkn-dwa.sbatch) | [cngrb2dg](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/cngrb2dg) | [`4c01d9ddd`](https://github.com/ischlag/megatron-lm-research-baseline/tree/4c01d9ddd) |
| 4 | aurora-qkn-vresid | aurora-qkn | add `--value-residual` (ResFormer / value-residual learning, arXiv 2410.17897) | Aurora + QK norm + V-residual | 1e-2 | 2.165 | 2.006 | [`18-aurora-qkn-vresid.sbatch`](runs/18-aurora-qkn-vresid.sbatch) | [d1z7rsv7](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/d1z7rsv7) | [`62ba2b1a4`](https://github.com/ischlag/megatron-lm-research-baseline/tree/62ba2b1a4) |
| 5 | aurora-qkn-xsa | aurora-qkn | add `--exclusive-self-attention` (XSA, GQA-aware + torch.compile) | Aurora + QK norm + XSA | 1e-2 | 2.167 | 2.009 | [`12-aurora-qkn-xsa.sbatch`](runs/12-aurora-qkn-xsa.sbatch) | [mdmnb7mc](https://wandb.ai/ischlag/megatron-lm-research-baseline/runs/mdmnb7mc) | [`435b3cf2d`](https://github.com/ischlag/megatron-lm-research-baseline/tree/435b3cf2d) |
| 6 | aurora-xsa | aurora-lr1e-2 | add `--exclusive-self-attention` (XSA) | Aurora + XSA | 1e-2 | 2.183 | 2.023 | [`13-aurora-xsa.sbatch`](runs/13-aurora-xsa.sbatch) | [w6vimrkm](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/w6vimrkm) | [`e994bb632`](https://github.com/ischlag/megatron-lm-research-baseline/tree/e994bb632) |
| 7 | aurora-qkn | aurora-lr1e-2 | add `--qk-layernorm` (RMSNorm on Q, K) | Aurora + QK norm | 1e-2 | 2.185 | 2.025 | [`09-aurora-qkn.sbatch`](runs/09-aurora-qkn.sbatch) | [s5e3c4bm](https://wandb.ai/ischlag/megatron-lm-research-baseline/runs/s5e3c4bm) | [`f3816b24e`](https://github.com/ischlag/megatron-lm-research-baseline/tree/f3816b24e) |
| 8 | aurora-qkn-fp8 | aurora-qkn | add `--fp8-format e4m3 --fp8-recipe blockwise` (DeepSeek-V3) | Aurora + QK norm (FP8) | 1e-2 | 2.188 | 2.027 | [`11-aurora-qkn-fp8.sbatch`](runs/11-aurora-qkn-fp8.sbatch) | [guwuunre](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/guwuunre) | [`6db3a8f8b`](https://github.com/ischlag/megatron-lm-research-baseline/tree/6db3a8f8b) |
...
| 19 | adamw-lr5e-4 | adamw-lr1e-3 | LR 1e-3 -> 5e-4 | AdamW | 5e-4 | 2.363 | 2.199 | [`06-adamw-lr5e-4.sbatch`](runs/06-adamw-lr5e-4.sbatch) | [zzywif5m](https://wandb.ai/ischlag/lm-research-baseline-dev/runs/zzywif5m) | [`baseline-2026-05-08-a76e5bc`](https://github.com/ischlag/megatron-lm-research-baseline/tree/baseline-2026-05-08-a76e5bc) |
...
- **`aurora-qkn-xsa-vresid`** (new dense leader): Stacks two independent code paths
  on top of the XSA leader. XSA mutates the post-attention output (arXiv
  2603.09078); V-residual mutates V before core_attention (arXiv 2410.17897).
  Solo deltas vs old leader: XSA −0.016, V-residual −0.019. Stacked: −0.032
  (within rounding of additive, so the wins are independent). Throughput
  ~234 TFLOP/s/GPU matches XSA solo (V-residual is one extra add per
  attention layer, no measurable cost). Zero new parameters from V-residual;
  XSA is also parameter-free.

- **`aurora-qkn-dwa`**: Adds `--dwa` (DenseFormer / Depth-Weighted-Average,
  arXiv 2402.02622). The TransformerBlock keeps a buffer of every past block's
  output (block 0 = input embedding, block 1..n = layer outputs) and
  replaces each layer's output with a learnable weighted sum of all past
  outputs. n*(n+1) scalar parameters (~600 at 24 layers), init'd to identity.
  MBS dropped 16 → 8 to fit the past-block activation cache. Steady throughput
  ~190 TFLOP/s/GPU; most of the −28% tax vs dense leader is from the smaller
  MBS, not the DWA mixing itself. Composes cleanly with V-residual (different
  modules); a stacked `xsa-vresid+dwa` run was attempted but didn't fit
  inside the 55-min wall budget — rerun with longer time to confirm.

- **`aurora-qkn-vresid`**: Adds `--value-residual` (ResFormer / value-residual
  learning, arXiv 2410.17897). Layer 1 stashes its V tensor into a class-level
  slot on `Attention`; layers 2..N add it to their own V before core_attention.
  Training-only path; no extra parameters; one extra `add` per attention layer.
  Throughput ~257 TFLOP/s/GPU (−3% vs aurora-qkn ~265).
